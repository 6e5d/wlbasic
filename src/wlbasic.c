...
static void wlbasic(topdeco_configure)(void*,
...
void wlbasic(config_default)(Wlbasic(Config)* conf) {
	...
	conf->topdeco_listener.configure = wlbasic(topdeco_configure);
	conf->shell_listener.ping = handle_shell_ping;

	wlbasic(seat_default)(&conf->seat_listener);
	wlbasic(surface_default)(&conf->surface_listener);
	wlbasic(output_default)(&conf->output_listener);
	wlbasic(keyboard_default)(&conf->keyboard_listener);
	wlbasic(pointer_default)(&conf->pointer_listener);
	wlbasic(tablet_default)(&conf->tablet_listener);
	wlbasic(tabseat_default)(&conf->tabseat_listener);
	wlbasic(tabtool_default)(&conf->tabtool_listener);
	wlbasic(gepinch_default)(&conf->gepinch_listener);
}

static void wlbasic(init2)(Wlbasic()* wl) {
...
void wlbasic(deinit)(Wlbasic()* wlbasic) {
	if (wlbasic->tablet) {
